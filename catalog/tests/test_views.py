...
class AuthorCreateViewTest(TestCase):
    ...
    def test_must_be_logged_in_to_view_create_auth(self):
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.url.startswith('/accounts/login'))

    def test_logged_in_user_may_not_view_create_auth(self):
        self.client.login(username='testuser1', password='test')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 403)

    def test_valid_user_may_access_create_auth(self):
        self.client.login(username='testuser2', password='test')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)

    def test_initial_date_is_set_to_a_specific_value(self):
        date_value = '05/01/2018'
        self.client.login(username='testuser2', password='test')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.context['form'].initial['date_of_death'], date_value)
    ...
    def test_redirect_to_author_detail_view_once_new_author_is_created(self):
        self.client.login(username='testuser2', password='test')
